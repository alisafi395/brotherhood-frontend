...
export default function SignupScreen({ navigation }) {
  ...
  const { signup, loading } = useDevAuth();

  const handleSignupPress = async () => {
    try {
      await signup(email, password);
      navigation.navigate("Step1"); // change to your real screen name
    } catch (e) {
  ...
  const handleGooglePress = async () => {
    try {
      ...
      Alert.alert("Placeholder", "Google signup will be implemented later.");
    } finally {
  ...
  const handleFacebookPress = () => {
    Alert.alert("Placeholder", "Facebook signup will be implemented later.");
  };
